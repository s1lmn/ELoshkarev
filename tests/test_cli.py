"""Tests for the console interface."""
...
class FakeIO:
    """Simple in-memory IO for deterministic CLI tests."""
...
class ConsoleInterfaceTests(unittest.TestCase):
    def test_run_handles_full_flow_with_sorting(self) -> None:
        ...
        self.assertIn("Запись добавлена: {'id': 1, 'title': 'Refactoring', 'year': 1999}", joined_output)
        self.assertIn("Запись добавлена: {'id': 2, 'title': 'Clean Code', 'year': 2008}", joined_output)
        self.assertIn("1. {'id': 2, 'title': 'Clean Code', 'year': 2008}", joined_output)
